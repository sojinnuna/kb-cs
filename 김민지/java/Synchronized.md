...
- 멀티 스레드: 한 개의 프로그램을 실행하고 내부적으로 여러가지 작업 처리(`애플리케이션 내부에서의 멀티 태스킹`)
...
- 메서드 선언에 `synchronized` 키워드를 붙임
    ...
    - 동기화 메서드는 메소드 전체 내용이 임계 영역
        - 동기화 메소드를 실행하는 즉시, 해당 객체에 대한 잠금이 일어나고 스레드가 동기화 메소드의 실행을 종료하면 잠금이 풀림
        - 즉, 특정 객체의 동기화 메서드를 실행하는 동안에는 그 객체에 대한 다른 동기화 메서드도 잠금에 의해 막히게 됨
...
- `synchronized(공유 객체) { 임계 영역 }`
    ...
    - 클래스 인스턴스의 block 단위로 Lock을 걸게 됨
        - 특정 객체를 명시하면 해당 객체에만 Lock을 걸어, 해당 객체에 Lock을 거는 다른 block들끼리 Lock을 공유함
...
> `Blocking`: 자신의 수행 결과가 끝날 때까지 제어권을 갖고 있는 것  
> `Non-Blocking`: 자신이 호출되었을 때, 제어권을 바로 자신을 호출한 쪽으로 넘기고 자신을 호출한 쪽에서 다른 작업을 할 수 있도록 하는 것
<div style="text-align: center">
<img src ="https://velog.velcdn.com/images/jifrozen/post/5398ea48-1fb5-4317-afb7-f12c12cdac1c/image.png" width="250"/>blocking
<img src ="https://velog.velcdn.com/images/jifrozen/post/c5f38fed-680e-4448-b1ca-878586490b74/image.png" width="250"/>non-blocking
</div>
<br/>

### 2) Volatile

- 변수의 값을 모든 스레드에 동일하게 보이도록 보장함. 이를 통해 변수의 가시성 문제를 해결할 수 있음.
- 가시성 보장
  - volatile 키워드가 붙은 변수는 각 스레드의 캐시가 아닌, 메인 메모리에서 직접 읽고 씀.
  - 따라서 한 스레드가 volatile 변수를 변경하면, 다른 스레드들은 즉시 그 변경된 값을 읽을 수 있음
- 원자성 보장하지 않음
  - volatile 변수의 읽기/쓰기는 원자적이지만, 이보다 복잡한 연산(예: count++와 같은 복합 연산)은 원자적이지 않음. 따라서 이러한 연산에는 여전히 synchronized 블록이나 java.util.concurrent 패키지의 원자 클래스가 필요
...
```java
  public class VolatileExample extends Thread {
      ...
      public static void main(String[] args) throws InterruptedException {
          ...
          Thread.sleep(2000); // 2초 후에 스레드를 멈춤
          thread.stopRunning();
      }
  }
```
- volatile 키워드는 변수의 가시성을 보장하지만, 복잡한 원자적 연산에는 적합하지 않습니다.
- 주로 상태 플래그, 종료 플래그 등 단순히 값을 읽고 쓰는 상황에서 사용됩니다.
...
public class ThreadLocalExample {
    ...
    public static void main(String[] args) {
        Runnable task = () -> {
            // ThreadLocal 변수의 값을 읽고 설정함.
            int value = threadLocalValue.get();
            System.out.println(Thread.currentThread().getName() + " 초기 값: " + value);

            threadLocalValue.set(value + 1);
            System.out.println(Thread.currentThread().getName() + " 변경 후 값: " + threadLocalValue.get());
        };
